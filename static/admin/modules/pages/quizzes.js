...
export function createAdminQuizzesModule() {
  return {
    ...
    selectedQuizListItem() {
      const items = Array.isArray(this.quizzes?.items) ? this.quizzes.items : [];
      if (!items.length) return null;
      const selectedKey = String(this.quizListSelectionKey ||"").trim();
      return items.find((item) => String(item?.quiz_key ||"").trim() === selectedKey) || items[0];
    },

    selectQuizListItem(item) {
      const quizKey = String(item?.quiz_key ||"").trim();
      if (quizKey) {
        this.quizListSelectionKey = quizKey;
      }
    },
    ...
    normalizeQuizPublicInviteFilter(value) {
      return String(value ||"").trim() ==="enabled" ?"enabled" :"";
    },

    quizPublicInviteFilterValue() {
      return this.normalizeQuizPublicInviteFilter(this.filters.quizzes?.public_invite);
    },

    quizPublicInviteFilterButtonClass() {
      return this.quizPublicInviteFilterValue()
        ?"border-emerald-200 bg-emerald-50 text-emerald-700"
        :"border-transparent text-slate-500 hover:bg-blue-50/60 hover:text-slate-800";
    },

    async toggleQuizPublicInviteFilter() {
      const nextValue = this.quizPublicInviteFilterValue() ?"" :"enabled";
      this.filters.quizzes.public_invite = nextValue;
    ...
    async loadQuizzes({ quiet = false, source ="manual", previousSyncStatus ="", previousSyncJobId ="" } = {}) {
      ...
      const publicInviteFilter = this.quizPublicInviteFilterValue();
      if (publicInviteFilter) query.set("public_invite", publicInviteFilter);
      ...
      const items = Array.isArray(this.quizzes?.items) ? this.quizzes.items : [];
      if (items.length && !items.some((item) => String(item?.quiz_key ||"").trim() === String(this.quizListSelectionKey ||"").trim())) {
        this.quizListSelectionKey = String(items[0]?.quiz_key ||"").trim();
      }
