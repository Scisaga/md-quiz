...
- 涉及标签、徽标、状态 pill 等内联小组件时，图标必须小于或等于文字高度，不得用固定大字号撑高标签；后台 Material Symbols 优先使用 `.admin-r-pill__icon`，并确保规则能覆盖图标字体默认 `24px`。
- 涉及第三方品牌、客户端或工具 logo 时，必须核对真实来源和原始资源，不得用搜索缩略图、生成图、手绘近似图或不相关图标替代；落库前检查尺寸、裁剪、透明/白底和页面居中效果。
