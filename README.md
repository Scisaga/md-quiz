...
> 把一句「我想了解什么」，变成可发放、可评分、可复用的专业问卷。

MD Quiz 是面向智能体工作时代的提问与验证应用，它把 QML 问卷、Git 版本、邀约、作答、评分、结果留档和后台运维放在同一套系统中，既可以由负责人直接使用，也可以通过 MCP 接受 Agent 调用。

[![MD Quiz 20 秒界面演示](img/md-quiz-demo-20s.webp)](img/md-quiz-demo-20s.mp4)

[▶ 查看高清 MP4](img/md-quiz-demo-20s.mp4)

## 它解决什么问题

MD Quiz 解决的不是「没有问卷工具」，而是「知道自己要做什么决定，却很难快速问出好问题」。传统表单需要从空白页开始，通用模板不了解当前语境，通用 AI 生成题目后仍缺少正式发布、执行和结果留档的环节。

负责人只需明确「要了解谁、关注什么、怎样算合格」，即可从相近问题包开始，或通过领域 Skill 生成可修改的 QML 草稿。内容经过程序校验和人工确认后再固定版本、创建邀约并正式发放；回收后统一完成评分、Traits 汇总和综合分析。

核心原则：**模型负责提出，程序负责校验与执行，人负责授权与担责。** AI 评分和分析用于辅助了解，高影响决定始终由人复核并做出。

## 从意图到结果

1. **说重点**：说明任务对象、关注重点与判断标准。
2. **出问卷**：复用问题包，或由领域 Skill 生成 QML 草稿。
3. **确认版本**：校验问卷结构，由负责人预览、修改并确认版本。
4. **正式发放**：创建邀约，设置答题要求，通过链接或二维码触达参与者。
5. **评分分析**：处理客观题、主观题和 Traits，生成结构化结果与综合分析。
6. **留档复用**：保留问卷版本、题目、答案和评分结果，供回看、迭代与再次使用。
...
- 结果支持客观题、主观题和 `traits` 量表；可接入 LLM 做辅助判卷、结构化评分、综合分析和简历解析
...
`Hermes / VS Code / Codex` 的客户端配置示例见 [docs/reference/mcp.md](docs/reference/mcp.md)。
